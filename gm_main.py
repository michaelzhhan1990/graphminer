...
def gm_save_kcores(dest_dir, belief):
    gm_sql_save_table_to_file(db_conn, GM_CON_COMP, "node_id, component_id", \
                              os.path.join(dest_dir, "5-kores.csv"), ",");
...
def gm_pagerank(num_nodes, max_iterations=gm_param_pr_max_iter, \
    ...
    cur.execute("INSERT INTO %s " % norm_table +
                ...
                " (SELECT src_id \"node_id\", sum(weight) \"weight_sm\" FROM %s GROUP BY src_id) \"TAB2\" " % (
                GM_TABLE) +
...
def k_cores(num_nodes, k, dest_dir):
    cur = db_conn.cursor()
    print 'get k-cores'
    #gm_sql_print_table(db_conn,GM_NODE_DEGREES)

    temp_gmtable = "gmtable"
    gm_sql_table_drop_create(db_conn, temp_gmtable, "src_id integer, dst_id integer,weight real")
    cur.execute("Insert into %s " % temp_gmtable + "select * from %s " % GM_TABLE)
    db_conn.commit()

    temp_degree = "degree"
    gm_sql_table_drop_create(db_conn, temp_degree, "node_id integer,in_degree integer, out_degree integer")
    cur.execute("Insert into %s " % temp_degree + "select * from %s " % GM_NODE_DEGREES)
    db_conn.commit()

    temp_nodes = "nodes1"
    gm_sql_table_drop_create(db_conn, temp_nodes, "node_id integer")
    cur.execute("INSERT INTO %s " % temp_nodes + " SELECT * FROM %s " % GM_NODES)
    db_conn.commit()

    while True:

        temp = "ktable"
        gm_sql_table_drop_create(db_conn, temp, "node_id integer")
        cur.execute(
            "Insert into %s " % temp + " select node_id from %s " % GM_NODE_DEGREES + " WHERE in_degree < %s" % k)
        db_conn.commit()
        #print

        cur.execute(
            "Delete from %s" % GM_TABLE + " where src_id IN (select node_id from %s)" % temp + "  or dst_id in (select node_id from %s) " % temp)
        db_conn.commit()
        gm_node_degrees()

        check = "checktable"
        gm_sql_table_drop_create(db_conn, check, "in_degree integer")
        cur.execute(
            "insert into %s " % check + " Select in_degree from %s " % GM_NODE_DEGREES + " where in_degree < %s" % k)
        db_conn.commit()
        #print "check"
        #gm_sql_print_table(db_conn,check)
        cur.execute("select count(*) from %s " % check)
        num = cur.fetchone()[0]
        if num == 0:
            break

    #gm_sql_print_table(db_conn,GM_TABLE)
    #print 
    #gm_sql_print_table(db_conn,GM_NODE_DEGREES)
    ...
    if num_nodes != 0:
        ...
        gm_sql_print_table(db_conn, GM_CON_COMP)
        gm_sql_save_table_to_file(db_conn, GM_CON_COMP, "node_id, component_id", \
                                  os.path.join(dest_dir, "5-kores.csv"), ",");
    else:
        print "no 5 cores"

    gm_sql_table_drop_create(db_conn, GM_TABLE, "src_id integer, dst_id integer,weight real")
    cur.execute("Insert into %s " % GM_TABLE + "select * from %s " % temp_gmtable)
    gm_sql_table_drop_create(db_conn, GM_NODE_DEGREES, "node_id integer,in_degree integer, out_degree integer")
    cur.execute("Insert into %s " % GM_NODE_DEGREES + "select * from %s " % temp_degree)
    gm_sql_table_drop_create(db_conn, GM_NODES, "node_id integer")
    cur.execute("Insert into %s " % GM_NODES + "select * from %s " % temp_nodes)

    db_conn.commit()
    gm_sql_table_drop(db_conn, temp_gmtable)
    gm_sql_table_drop(db_conn, temp_degree)
    gm_sql_table_drop(db_conn, temp_nodes)
    cur.close()
...
def gm_connected_components(num_nodes):
    ...
    while True:
        ...
        cur.execute("INSERT INTO %s " % temp_table +
                    ...
                    " SELECT src_id \"node_id\", MIN(component_id) \"component_id\" FROM %s, %s" % (
...
def gm_eigen_QR_decompose(T, n, Q, R):
    ...
    for i in range(1, n):
        ...
        cur.execute(
            'UPDATE %s' % (G) + ' SET value = %s WHERE row_id = %s AND col_id = %s' % (str(c), str(i + 1), str(i + 1)))
...
def main():
    ...
    try:
        ...
        print "=== This task takes " + str(time.time() - myTimer) + " seconds. ===\n"
        ...
        myTimer = time.time()
        k_cores(num_nodes, 5, args.dest_dir)
        print "=== This task takes " + str(time.time() - myTimer) + " seconds. ===\n"
        myTimer = time.time()
        gm_connected_components(num_nodes)
        print "=== This task takes " + str(time.time() - myTimer) + " seconds. ===\n"
        ...
        gm_db_bubye(db_conn)
        print "=== The whole program takes " + str(time.time() - totalTimer) + " seconds. ===\n";
    except:
