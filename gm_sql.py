...
def gm_sql_unique_index(db_conn,table_name,col_name,index_name):
    ...
    cur.execute("DROP INDEX IF EXISTS %s" % index_name)
    db_conn.commit()
    cur.execute("CREATE UNIQUE INDEX %s " % index_name+" ON %s " %(table_name)+"( %s" % col_name +")")
    db_conn.commit()
    cur.close()

def gm_sql_dropindex(db_conn,table_name,index_name):
    cur = db_conn.cursor()
    cur.execute("DROP INDEX IF EXISTS %s" % index_name)#+" ON %s" % (table_name)+"( %s" % col_name +")")
    #cur.execute("DROP INDEX %s" % index_name+" ON %s" % table_name)
    db_conn.commit()
    cur.close()
